...
function solution(blocks, rule){
  ...
  for (let block of blocks){
    answer.push(checkBlockOrder(block, rule));
  }
  return answer;
}

function checkBlockOrder(block, rule){
  let element = rule.indexOf(rule[0]);
  for (let word of block){
    if (rule.includes(word)){
      if (element > rule.indexOf(word)){
        return '불가능';
      }
      element = rule.indexOf(word);
    }
  }
  return '가능';
}
